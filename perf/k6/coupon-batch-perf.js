 ...
 * 2. scheduler_trigger: 테스트 시작 후 10초 뒤에 만료 배치 강제 1회 실행
 *
 * 지표 구간
 * ----------------------------------------------------------------------------
 * pre_batch_*     : 배치 트리거 전 구간
 * during_batch_*  : 배치 트리거 직후 BATCH_MEASURE_SECONDS 동안
...
export function setup() {
    ...
    console.log(`[setup] batch starts at T+${BATCH_START_SECONDS}s, during window=${BATCH_MEASURE_SECONDS}s.`);
...
export function triggerScheduler(data) {
    ...
    console.log('[triggerScheduler] 마감 처리 배치 실행 요청 보냄...');

    const res = http.post(`${BASE_URL}/api/test/coupons/expire-batch-trigger`, null, {
        headers: {
            'Authorization': `Bearer ${data.adminToken}`,
        },
        timeout: '10m',  // 배치 완료까지 최대 10분 대기 (k6 기본값 60s 초과 방지)
    });
