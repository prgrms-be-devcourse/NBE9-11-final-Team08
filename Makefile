...
# 부하 테스트(k6) - 부하/관측 stack만 perf/ 에 둔다.
# 측정 대상 서버는 deploy compose 가 담당: dev(make dev + IntelliJ backend) 또는 prod(make prod).
PERF_DIR     := perf
...
perf: ## [부하] k6 실행 (host.docker.internal:8080 타격) + influx/grafana 기동
...
perf-down: ## [부하] 부하 stack 정리 (볼륨 유지 → 대시보드/이력 보존)
	cd $(PERF_DIR) && $(PERF_DC) --profile k6 down
	@echo "✅ 부하 stack 내림 (볼륨 유지)."

perf-reset: ## [부하] 부하 stack 완전 초기화 (볼륨 삭제 → 대시보드 재import)
