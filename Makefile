...
.DEFAULT_GOAL := help
.PHONY: help dev dev-down dev-logs db-reset edge edge-down prod prod-pull prod-down prod-logs ps down-all perf perf-down perf-logs perf-server perf-client perf-split-down
...
## ─────────────── 부하 테스트 — 한 머신 모드 ───────────────
# PERF_SCRIPT 값 지정하여 원하는 스크립트 테스트 가능
perf: ## [부하] 올인원 기동(db+app+influx+grafana) 후 k6 실행, 결과는 InfluxDB→Grafana
	$(call run-perf,up -d --build influxdb grafana)
	$(call run-perf,up -d --build --wait app)
	$(call run-perf,run --rm k6 run --out influxdb=http://influxdb:8086/k6 /scripts/$(PERF_SCRIPT))
	@echo "📊 Grafana: http://localhost:3000   (정리: make perf-down)"

perf-down: ## [부하] 한 머신 모드 컨테이너/볼륨 정리
...
perf-split-down: ## [분리] 서버+부하 stack 모두 정리
