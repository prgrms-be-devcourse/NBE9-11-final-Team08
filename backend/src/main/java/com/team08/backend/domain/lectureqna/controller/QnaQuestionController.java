...
public class QnaQuestionController {
    ...
    public ApiResponse<Page<QnaQuestionResponse>> getQna(
            @PathVariable Long lectureId,
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        return ApiResponse.success(qnaQuestionService.getQuestionsNAnswers(lectureId, pageable));
    }

    @Operation(summary = "QnA 질문 작성")
    @PostMapping("/api/lectures/{lectureId}/qna/questions")
    public ApiResponse<QnaQuestionResponse> createQuestion(
            @PathVariable Long lectureId,
            @Valid @RequestBody QnaQuestionRequest request,
            @AuthenticationPrincipal LoginUserPrincipal principal) {
        return ApiResponse.success(
                qnaQuestionService.createQuestion(lectureId, principal.user().id(), request.title(), request.content()));
    }

    @Operation(summary = "QnA 질문 수정")
    @PutMapping("/api/qna/questions/{questionId}")
    public ApiResponse<QnaQuestionResponse> updateQuestion(
            @PathVariable Long questionId,
            @Valid @RequestBody QnaQuestionRequest request,
            @AuthenticationPrincipal LoginUserPrincipal principal) {
        return ApiResponse.success(
                qnaQuestionService.updateQuestion(questionId, principal.user().id(), request.title(), request.content()));
    }
